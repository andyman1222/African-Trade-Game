...
//AfricanTradeGameinTerminal() { }
	public static void main(String args[]) {
		...
		System.out.println("\n"); 
		payGold = Math.round(salt / 2)*100; 
		System.out.println((new StringBuilder()).append("\nYou meet some border guards. They say that you have to pay ").append((salt / 2)*100).append(" gold to pass.").toString()); 
		if(payGold > gold) { 
	...
	static void trade() { 
		...
		do { 
			...
			try { 
				...
				traderr.userSalt = payment;
				traderr.userGold = gold; 
				if(payment > salt) 
			...
			System.out.println((new StringBuilder()).append("\nYou leave the salt on a plate and shop around (shopping coming soon). You come back and see ").append(traderr.tradeGold).append(" gold on your plate. accept, leave, or change? choose one.").toString()); 
			asker = scanner.nextLine(); 
			if(asker.equals("accept")) { 
			gold = traderr.tradeGold + gold; 
			salt -= payment; 
	...
	static trader traderr = new trader(0,0);
}
class trader{
	...
	//for future use- multiple traders
	public int hasGold = (int)Math.random()*1000+100;

	public trader(int userGold, int userSalt){
		this.userGold = userGold;
		this.userSalt = userSalt;
	}
	void generateGold(){

		//will be generated upon init in the future
		hasGold = (int)Math.random()*1000+100;
		wealth = Math.random()*20;
		//

		tradeGold = (int)Math.round(100*(double)userSalt * (double)userGold* wealth);
	}
}